...
image_size = 19 # image width and height
...
with open(output_file, 'r') as file:
    ...
    if len(input_pe_command_occurences) == 2 and abs(input_pe_command_occurences[0] - input_pe_command_occurences[1]) == 2:
        mid = int((input_pe_command_occurences[0] + input_pe_command_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_pe_command) + ")\n"
        #print(lines[mid])
    else:
        print("ERROR: input_pe_command_occurences not found")
        exit
    if len(input_command_iact_occurences) == 2 and abs(input_command_iact_occurences[0] - input_command_iact_occurences[1]) == 2:
        mid = int((input_command_iact_occurences[0] + input_command_iact_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_command_iact) + "),\n"
        #print(lines[mid])
    else:
        print("ERROR: input_command_iact_occurences not found")
        exit
    if len(input_command_wght_occurences) == 2 and abs(input_command_wght_occurences[0] - input_command_wght_occurences[1]) == 2:
        mid = int((input_command_wght_occurences[0] + input_command_wght_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_command_wght) + ")\n"
        #print(lines[mid])
    else:
        print("ERROR: input_command_wght_occurences not found")
        exit
    if len(input_command_psum_occurences) == 2 and abs(input_command_psum_occurences[0] - input_command_psum_occurences[1]) == 2:
        mid = int((input_command_psum_occurences[0] + input_command_psum_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_command_psum) + "),\n"
        #print(lines[mid])
    else:
        print("ERROR: input_command_psum_occurences not found")
        exit
    if len(input_read_offset_iact_occurences) == 2 and abs(input_read_offset_iact_occurences[0] - input_read_offset_iact_occurences[1]) == 2:
        mid = int((input_read_offset_iact_occurences[0] + input_read_offset_iact_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_read_offset_iact) + "),\n"
        #print(lines[mid])
    else:
        print("ERROR: input_read_offset_iact_occurences not found")
        exit
    if len(input_read_offset_wght_occurences) == 2 and abs(input_read_offset_wght_occurences[0] - input_read_offset_wght_occurences[1]) == 2:
        mid = int((input_read_offset_wght_occurences[0] + input_read_offset_wght_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_read_offset_wght) + ")\n"
        #print(lines[mid])
    else:
        print("ERROR: input_read_offset_wght_occurences not found")
        exit
    if len(input_read_offset_psum_occurences) == 2 and abs(input_read_offset_psum_occurences[0] - input_read_offset_psum_occurences[1]) == 2:
        mid = int((input_read_offset_psum_occurences[0] + input_read_offset_psum_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_read_offset_psum) + "),\n"
        #print(lines[mid])
    else:
        print("ERROR: input_read_offset_psum_occurences not found")
        exit
    if len(input_update_offset_iact_occurences) == 2 and abs(input_update_offset_iact_occurences[0] - input_update_offset_iact_occurences[1]) == 2:
        mid = int((input_update_offset_iact_occurences[0] + input_update_offset_iact_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_update_offset_iact) + "),\n"
        #print(lines[mid])
    else:
        print("ERROR: input_update_offset_iact_occurences not found")
        exit
    if len(input_update_offset_wght_occurences) == 2 and abs(input_update_offset_wght_occurences[0] - input_update_offset_wght_occurences[1]) == 2:
        mid = int((input_update_offset_wght_occurences[0] + input_update_offset_wght_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_update_offset_wght) + ")\n"
        #print(lines[mid])
    else:
        print("ERROR: input_update_offset_wght_occurences not found")
        exit
    if len(input_update_offset_psum_occurences) == 2 and abs(input_update_offset_psum_occurences[0] - input_update_offset_psum_occurences[1]) == 2:
        mid = int((input_update_offset_psum_occurences[0] + input_update_offset_psum_occurences[1])/2)
        lines[mid] = "        (" + ",".join(list_input_update_offset_psum) + "),\n"
        #print(lines[mid])
    else:
        print("ERROR: input_update_offset_psum_occurences not found")
        exit
    if len(output_read_offset_occurences) == 2 and abs(output_read_offset_occurences[0] - output_read_offset_occurences[1]) == 6:
        start = int(min(output_read_offset_occurences[0], output_read_offset_occurences[1])) + 1
        for i in range(size_y):
            if i == size_y - 1:
                lines[start + i] = "        (" + ",".join(list_output_read_offset[i]) + ")\n"
            else:
                lines[start + i] = "        (" + ",".join(list_output_read_offset[i]) + "),\n"
            #print(lines[start + i])
    else:
        ...
        exit
    if len(output_update_offset_occurences) == 2 and abs(output_update_offset_occurences[0] - output_update_offset_occurences[1]) == 6:
        start = int(min(output_update_offset_occurences[0], output_update_offset_occurences[1])) + 1
        for i in range(size_y):
            if i == size_y - 1:
                lines[start + i] = "        (" + ",".join(list_output_update_offset[i]) + ")\n"
            else:
                lines[start + i] = "        (" + ",".join(list_output_update_offset[i]) + "),\n"
            #print(lines[start + i])
    else:
        ...
        exit
    if len(output_pe_command_occurences) == 2 and abs(output_pe_command_occurences[0] - output_pe_command_occurences[1]) == 6:
        start = int(min(output_pe_command_occurences[0], output_pe_command_occurences[1])) + 1
        for i in range(size_y):
            if i == size_y - 1:
                lines[start + i] = "        (" + ",".join(list_output_pe_command) + ")\n"
            else:
                lines[start + i] = "        (" + ",".join(list_output_pe_command) + "),\n"
            #print(lines[start + i])
    else:
        ...
        exit
    if len(output_command_occurences) == 2 and abs(output_command_occurences[0] - output_command_occurences[1]) == 6:
        start = int(min(output_command_occurences[0], output_command_occurences[1])) + 1
        for i in range(size_y):
            if i == size_y - 1:
                lines[start + i] = "        (" + ",".join(list_output_command[i]) + ")\n"
            else:
                lines[start + i] = "        (" + ",".join(list_output_command[i]) + "),\n"
            #print(lines[start + i])
    else:
    ...
    if len(command_length_occurences) == 2 and abs(command_length_occurences[0] - command_length_occurences[1]) == 2:
        ...
        lines[mid] = "        " + str(len(list_input_pe_command)) + "\n"
        #print(lines[mid])
    else:
